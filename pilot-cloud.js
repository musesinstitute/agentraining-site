(function () {
  ...
  function showGate(message) {
    ...
    if (!gate) {
      ...
      gate.querySelector('#pilot-auth-toggle').addEventListener('click', event => {
        const password = gate.querySelector('#pilot-auth-password');
        const showing = password.type === 'text';
        password.type = showing ? 'password' : 'text';
        event.currentTarget.textContent = showing ? t('👁 Show','👁 显示') : t('🙈 Hide','🙈 隐藏');
      });
      gate.querySelector('#pilot-auth-form').addEventListener('submit', async event => {
        ...
        const message = gate.querySelector('#pilot-auth-message');
        ...
        status.textContent = '';
        message.textContent = t('Checking your account and secure access…','正在核对账号和安全访问凭证…');
        try {
          const loggedIn = await window.netlifyIdentity.gotrue.login(email, password, true);
          await loggedIn.jwt();
          message.textContent = t('Sign-in verified. Opening your workspace…','登录验证成功，正在进入工作区…');
          if (finishReady) finishReady(loggedIn);
          else window.location.reload();
        } catch (error) {
          const friendlyError = authError(error);
        ...
        if (user) {
          Promise.resolve(user.jwt())
            ...
            .catch(async () => {
              ...
              removeAccountControls();
              showGate(t('Old test session cleared. Sign in with your current invited account.','旧测试登录已清除。请使用当前受邀请的账号登录。'));
            });
        } else {
  ...
  async function resetExpiredSession() {
    ...
    finishReady = null;
    removeAccountControls();
    showGate(t("This browser saved an old test session. Sign in with your current invited account.","此浏览器保存了旧的测试登录。请使用当前受邀请的账号重新登录。"));
  }
