...
async function callBatch(record,{count,difficulty,startNumber,existingFingerprints,timeoutMs=18000}){
 ...
 const source=compactSource(record.content,10000);
 const diff=['Basic','Intermediate','Advanced','Mixed'].includes(difficulty)?difficulty:'Mixed';
 const existing=[...existingFingerprints].slice(-120).join('\n');
 const input=[
  ...
  'Use a practical mix: about 60% multiple choice, 20% true/false, 20% scenario/application questions.',
