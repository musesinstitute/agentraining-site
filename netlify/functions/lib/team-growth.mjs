...
function repeatedStuckSignal(sessions, assignmentsById, assignments) {
  ...
  for (const session of sessions) {
    ...
    if (!scenario || score === null || score >= PASS_THRESHOLD) continue;
  ...
  const qualifying = [...byScenario.entries()]
    .filter(([, failures]) => failures.length >= STUCK_MIN_ATTEMPTS)
 ...
 * @param {object[]} input.assignments       pilot assignments
 * @param {object[]} input.assignmentEvents  pilot assignment-events (type 'assigned' | 'completed')
 * @param {string|number|Date} input.now
 */
export function computeTeamGrowth({ members = [], sessions = [], assignments = [], assignmentEvents = [], now = Date.now() } = {}) {
  ...
  const completedIds = new Set(assignmentEvents.filter(event => event?.type === 'completed').map(event => event.assignmentId));
  ...
  for (const member of roster.values()) {
    ...
    const signals = [
      ...
      openAssignmentSignal(memberAssignments, completedIds, assignedAtById, nowMs),
  ...
  // New-member count needs a reliable join date for every member; without it
  // the count is reported as unavailable instead of guessed.
  const joinDatesKnown = roster.size > 0 && [...roster.values()].every(member => time(member.joinedAt) !== null);
  const newMembers = joinDatesKnown
    ? [...roster.values()].filter(member => nowMs - time(member.joinedAt) <= NEW_MEMBER_DAYS * DAY_MS).length
    : null;
  ...
  return {
    summary: {
      ...
      practicedThisWeek,
      newMembers,
      newMembersAvailable: joinDatesKnown,
      weekStart: new Date(weekStart).toISOString()
    },
    focus: candidates.slice(0, FOCUS_LIMIT),
    rules: {
      passThreshold: PASS_THRESHOLD,
