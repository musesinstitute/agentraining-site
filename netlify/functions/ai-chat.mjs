...
export default async function handler(req) {
  try {
    ...
    if (!response.ok) {
      ...
      return reply(502, { error: payload?.error?.message || 'AI conversation request failed.' });
    }

    const text = extractOutputText(payload);
    if (!text) return reply(502, { error: 'AI conversation returned no text.' });
    return reply(200, { text, model, role, lang });
  } catch (error) {
