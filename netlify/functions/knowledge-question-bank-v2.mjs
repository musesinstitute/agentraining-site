...
async function reviewUnsupportedDuplicates(rows,current){
  ...
  const out=await aiCall(input,'Compact valid JSON only. This is an independent full Quality Gate review. No Evidence, No Authority.',3000,VERIFY_TIMEOUT_MS);
...
async function verifyCandidates(candidates,asgs,current){
  ...
  const out=await aiCall(input,'Compact valid JSON only. Be conservative. Scenario is not synonymous with Application. No duplicate rejection without evidence.',3000,VERIFY_TIMEOUT_MS),vm=new Map((out.parsed?.verdicts||[]).map(v=>[Number(v.candidateId),v])),accepted=[],rejected=[],unsupported=[];
  ...
  if(unsupported.length){
    let secondary,secondaryError='';try{secondary=await reviewUnsupportedDuplicates(unsupported,current)}catch(e){secondary=new Map();secondaryError=clean(e?.message||'Secondary review unavailable.',160)}
