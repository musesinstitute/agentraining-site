...
function safeAiFailure(stage,category,model,raw){const value=String(raw||'');return{stage,errorCategory:category,model:clean(model,80),responseBytes:Buffer.byteLength(value,'utf8'),responseSha256:createHash('sha256').update(value).digest('hex')}}
function json(raw,stage,model){let t=String(raw||'').trim().replace(/^```(?:json)?\s*/i,'').replace(/```\s*$/,'').trim();try{return JSON.parse(t)}catch{}const a=t.indexOf('{'),b=t.lastIndexOf('}');if(a>=0&&b>a)try{return JSON.parse(t.slice(a,b+1))}catch{}throw Object.assign(new Error('AI returned unreadable JSON. Retry this batch.'),{status:502,aiFailure:safeAiFailure(stage,'unreadable_json',model,raw)})}
...
function priorQuestions(current){return current.slice(-80).map(q=>({questionId:q.id,knowledgePointId:q.knowledgePointId,depth:q.depth,type:q.type,question:q.question,explanation:q.explanation}))}
function verifiedDuplicate(v,current){
  const id=clean(v?.duplicateOfQuestionId,40),prior=current.find(q=>String(q.id)===id),claimed=clean(v?.priorQuestionText,1200),evidence=clean(v?.duplicateEvidence,500);
  if(!prior||!claimed||fp(claimed)!==fp(prior.question)||evidence.length<12)return null;
  return{priorQuestionId:prior.id,priorQuestionText:clean(prior.question,1200),duplicateEvidence:evidence};
}
function rejection(q,status,reason,evidence,reviewStage){return{knowledgePointId:q.knowledgePointId,question:clean(q.question,220),reasons:[clean(status||'needs_review',80)],reviewReason:clean(reason||'Verifier did not accept.',300),...(evidence||{}),...(reviewStage?{reviewStage}:{})}}
async function reviewUnsupportedDuplicates(rows,current){
  if(!rows.length)return new Map();
  const input=['Secondary independent review for unsupported duplicate verdicts.','The first verifier claimed duplicate but did not provide a valid link to an actual prior accepted question. Re-check ALL quality requirements independently: source grounding, answer/logic, phase depth, format, sensitive claims, and semantic uniqueness.','Accept only when the candidate passes every requirement. If duplicate, copy the exact prior question ID and exact prior question text and explain the shared fact and cognitive operation. If duplication cannot be tied to a listed prior question, return needs_review, never rejected_duplicate.','CANDIDATES:\n'+JSON.stringify(rows.map(x=>({candidateId:x.candidateId,question:x.question,canonicalPoint:x.canonicalPoint,phase:x.phase,depthEligibilityScore:x.depthEligibilityScore,firstReviewReason:x.firstReviewReason}))),'PRIOR ACCEPTED:\n'+JSON.stringify(priorQuestions(current)),'Return ONLY JSON: {"verdicts":[{"candidateId":0,"status":"accepted|rejected_duplicate|rejected_logic|rejected_grounding|rejected_sensitive_claim|rejected_wrong_knowledge_point|rejected_depth|rejected_format|needs_review","reason":"brief reason","duplicateOfQuestionId":"required only for rejected_duplicate","priorQuestionText":"exact prior question text, required only for rejected_duplicate","duplicateEvidence":"shared fact and cognitive operation, required only for rejected_duplicate"}]}'].join('\n\n');
  const out=await aiCall(input,'Compact valid JSON only. This is an independent full Quality Gate review. No Evidence, No Authority.',3000,VERIFY_TIMEOUT_MS,'secondary_duplicate_review');
  return new Map((out.parsed?.verdicts||[]).map(v=>[Number(v.candidateId),v]));
}
async function verifyCandidates(candidates,asgs,current){
  if(!candidates.length)return{accepted:[],rejected:[]};
  const m=new Map(asgs.map(a=>[a.point.id,a])),payload=candidates.map((q,i)=>({candidateId:i,question:q,canonicalPoint:m.get(q.knowledgePointId)?.point||null,phase:m.get(q.knowledgePointId)?.phase||null,depthEligibilityScore:m.get(q.knowledgePointId)?.depthEligibilityScore||0})),prior=priorQuestions(current);
  const input=['Independent enterprise Question Quality Gate.','Infer actual cognitive work; do not trust the declared depth or type.','For Q21-35 accept only genuine Understanding: comparison, interpretation, meaningful distinction, relationship, supported calculation, or explanation. Reject direct recall/paraphrase.','For Q36-50 accept only genuine Application: source-supported decision, choice among alternatives, supported calculation, or application of multiple source attributes/rules to a new case.','Scenario is not synonymous with Application. If removing the client wrapper leaves a direct recall question, reject as rejected_depth.','Reject semantic restatements of prior questions even when wording/type/difficulty changes. Same fact/list/number retrieved the same way = rejected_duplicate.','A rejected_duplicate verdict MUST copy the exact prior question ID and exact prior question text from PRIOR ACCEPTED, and explain the shared fact and cognitive operation. Without all three, use needs_review.','Accept only when K-ID, answer/options/explanation, arithmetic/logic, source fidelity, semantic uniqueness and format are sound. Judge fidelity against canonical source point, not outside knowledge. Ambiguous support => needs_review. Sensitive claims must be source-framed.','CANDIDATES:\n'+JSON.stringify(payload),'PRIOR ACCEPTED:\n'+JSON.stringify(prior),'Return ONLY JSON: {"verdicts":[{"candidateId":0,"status":"accepted|rejected_duplicate|rejected_logic|rejected_grounding|rejected_sensitive_claim|rejected_wrong_knowledge_point|rejected_depth|rejected_format|needs_review","reason":"brief reason","duplicateOfQuestionId":"required only for rejected_duplicate","priorQuestionText":"exact prior question text, required only for rejected_duplicate","duplicateEvidence":"shared fact and cognitive operation, required only for rejected_duplicate"}]}'].join('\n\n');
  const out=await aiCall(input,'Compact valid JSON only. Be conservative. Scenario is not synonymous with Application. No duplicate rejection without evidence.',3000,VERIFY_TIMEOUT_MS,'primary_quality_gate'),vm=new Map((out.parsed?.verdicts||[]).map(v=>[Number(v.candidateId),v])),accepted=[],rejected=[],unsupported=[],stageFailures=[];
  candidates.forEach((q,i)=>{const v=vm.get(i);if(v?.status==='accepted')accepted.push(q);else if(v?.status==='rejected_duplicate'){const e=verifiedDuplicate(v,current);if(e)rejected.push(rejection(q,'rejected_duplicate',v.reason,e,'primary'));else unsupported.push({...payload[i],firstReviewReason:clean(v?.reason,300)});}else rejected.push(rejection(q,v?.status,v?.reason,null,'primary'));});
  if(unsupported.length){
    let secondary,secondaryError='';try{secondary=await reviewUnsupportedDuplicates(unsupported,current)}catch(e){secondary=new Map();secondaryError=clean(e?.message||'Secondary review unavailable.',160);if(e?.aiFailure)stageFailures.push(e.aiFailure)}
    for(const row of unsupported){const q=candidates[row.candidateId],v=secondary.get(row.candidateId);if(v?.status==='accepted')accepted.push(q);else if(v?.status==='rejected_duplicate'){const e=verifiedDuplicate(v,current);rejected.push(e?rejection(q,'rejected_duplicate',v.reason,e,'secondary'):rejection(q,'needs_review','Secondary duplicate verdict lacked verifiable prior-question evidence.',null,'secondary'));}else rejected.push(rejection(q,v?.status,v?.reason||'Secondary review did not accept.',null,'secondary'));}
    if(secondaryError)for(const r of rejected.filter(r=>r.reviewStage==='secondary'&&r.reviewReason==='Secondary review did not accept.'))r.reviewReason='Secondary review unavailable; candidate requires review.';
  }
  return{accepted,rejected,stageFailures};
}
function failureRecord(error,bank,batchRequestId,retryAttempt){const f=error?.aiFailure||{};return{bankId:bank.id,batchRequestId:clean(batchRequestId,100),stage:clean(f.stage||'generate_batch',80),retryAttempt:Math.max(0,parseInt(retryAttempt)||0),timestamp:new Date().toISOString(),errorCategory:clean(f.errorCategory||'batch_exception',80),...(f.model?{model:clean(f.model,80)}:{}),...(Number.isFinite(f.responseBytes)?{responseBytes:f.responseBytes}:{}),...(f.responseSha256?{responseSha256:clean(f.responseSha256,64)}:{})}}
async function persistInterrupted(store,key,bank,error,batchRequestId,retryAttempt){const failure=failureRecord(error,bank,batchRequestId,retryAttempt),history=[...(bank.failureHistory||[]),failure].slice(-20),updated={...bank,status:'interrupted',recoverable:true,lastFailure:failure,failureHistory:history,updatedAt:failure.timestamp};await store.setJSON(key,updated);error.recovery={bankId:bank.id,knowledgeId:bank.knowledgeId,totalQuestions:(bank.questions||[]).length,targetQuestions:bank.targetQuestions,status:updated.status,lastFailure:failure};return updated}
