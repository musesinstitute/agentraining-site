export default async (_request: Request, context: any) => {
  ...
  html = html.replace(
    '<b data-en="Supported transcript files:" data-zh="支援的逐字稿文件：">Supported transcript files:</b> TXT, MD, VTT, SRT · <span data-en="up to 5 MB (up to 500,000 characters). Direct video upload and automatic transcription are not included in this Pilot." data-zh="最大5MB（最多500,000字元）。本Pilot暫不包括直接上傳影片及自動轉錄。">up to 5 MB (up to 500,000 characters). Direct video upload and automatic transcription are not included in this Pilot.</span>',
    '<b data-en="Supported company files:" data-zh="支援的企業文件：">Supported company files:</b> PDF, Word (.docx), PowerPoint (.pptx), TXT, MD, VTT, SRT · <span data-en="up to 10 MB per file (up to 500,000 extracted characters). The browser extracts readable text for manager review before saving. Direct video upload and automatic transcription are not included in this Pilot." data-zh="每份最大10MB（擷取文字最多500,000字元）。瀏覽器會先擷取可讀文字供主管檢查，再儲存。本Pilot暫不包括直接上傳影片及自動轉錄。">up to 10 MB per file (up to 500,000 extracted characters). The browser extracts readable text for manager review before saving. Direct video upload and automatic transcription are not included in this Pilot.</span>'
  );

  html = html.replace(
    '<label for="transcriptFile" data-en="Upload text transcript (optional)" data-zh="上傳文字逐字稿（可選）">Upload text transcript (optional)</label><input id="transcriptFile" type="file" accept=".txt,.md,.vtt,.srt,text/plain"><small data-en="Accepted: TXT, MD, VTT, SRT up to 5 MB. The text is reviewed before saving." data-zh="接受TXT、MD、VTT、SRT，最大5MB；儲存前可先檢查文字。">Accepted: TXT, MD, VTT, SRT up to 5 MB. The text is reviewed before saving.</small>',
  ...
  html = html.replace(
    "document.getElementById('transcriptFile').addEventListener('change',async function(){var file=this.files&&this.files[0];if(!file)return;if(file.size>MAX_TRANSCRIPT_FILE_BYTES){setFormStatus(tr('Transcript file must be 5 MB or smaller.','逐字稿文件必須為5MB或以下。'),'error');this.value='';return}try{var text=await file.text();if(text.length>MAX_SOURCE_CHARS){setFormStatus(oversizedSourceMessage(text.length),'error');this.value='';return}document.getElementById('sourceContent').value=text;document.getElementById('sourceContent').dispatchEvent(new Event('input'));setFormStatus(tr('Loaded: ','已載入：')+file.name+tr('. Review the text before saving.','。請在儲存前檢查文字。'),'ok')}catch(error){setFormStatus(tr('Transcript could not be read.','無法讀取逐字稿文件。'),'error')}});",
    ''
  );

  // Long Training Content Fast Track: the base page's own textarea/limit
  // copy (maxlength, charCount, "0 / 500,000") already matches the new
  // 500,000-character Pilot limit (see knowledge.html) - the enhanced
  // PDF/DOCX/PPTX extraction handler below is fixed separately so it stops
  // silently slicing extracted text to the old 30,000-character cap.
...
<script>
(function(){
 var MAX_FILE_BYTES=10*1024*1024,MAX_TEXT_CHARS=500000;
 ...
 input.addEventListener('change',async function(){var file=this.files&&this.files[0];if(!file)return;if(file.size>MAX_FILE_BYTES){setFormStatus(tr('File must be 10 MB or smaller.','文件必須為10MB或以下。'),'error');this.value='';return}setFormStatus(tr('Reading “'+file.name+'”…','正在讀取「'+file.name+'」……'),'');try{var extracted=cleanExtracted(await extractFile(file));if(!extracted)throw new Error(tr('No readable text was found in this file.','此文件沒有找到可讀文字。'));if(extracted.length>MAX_TEXT_CHARS){setFormStatus(tr('This training source exceeds the current Pilot limit of 500,000 characters (extracted '+extracted.length.toLocaleString()+' characters). The source was NOT truncated or partially loaded.','此培训资料超过当前 Pilot 支持的 500,000 字符容量（擷取到 '+extracted.length.toLocaleString()+' 字符）。系统没有截断或部分载入该资料。'),'error');this.value='';return}var content=document.getElementById('sourceContent');content.value=extracted;content.dispatchEvent(new Event('input'));document.getElementById('sourceType').value='document_notes';var title=document.getElementById('sourceTitle');if(!title.value.trim())title.value=file.name.replace(/\\.(pdf|docx|pptx|ppt|txt|md|vtt|srt)$/i,'').slice(0,240);content.scrollIntoView({behavior:'smooth',block:'center'});setFormStatus(tr('Loaded: ','已載入：')+file.name+tr('. Review the extracted text before saving.','。請在儲存前檢查擷取的文字。'),'ok')}catch(error){console.error('Company file import failed',error);setFormStatus((error&&error.message)||tr('This file could not be read.','無法讀取此文件。'),'error');this.value=''}});
})();
</script>`;

  html = html.replace('</body>', enhancedUpload + '\n<script src="/knowledge-manager-review.js?v=20260904-review1"></script>\n</body>');
