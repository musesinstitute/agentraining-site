export default async (_request: Request, context: any) => {
  ...
  html = html.replace(
    ...
    "const learnerRoster=new Map();cloudProfiles.forEach(profile=>{const email=String(profile.learnerEmail||'').trim().toLowerCase();if(email)learnerRoster.set(email,profile.preferredName||email)});cloudAssignments.forEach(item=>{const email=String(item.assignedTo||item.learner||'').trim().toLowerCase();if(email&&!learnerRoster.has(email))learnerRoster.set(email,item.learner||email)});cloudResults.forEach(item=>{const email=String(item.learner||'').trim().toLowerCase();if(email&&!learnerRoster.has(email))learnerRoster.set(email,item.learnerName||email)});try{const rosterToken=await PilotCloud.currentUser().jwt();const rosterResponse=await fetch('/.netlify/functions/pilot-roster',{headers:{authorization:'Bearer '+rosterToken}});if(rosterResponse.ok){const rosterData=await rosterResponse.json();(rosterData.learners||[]).forEach(member=>{const email=String(member.email||'').trim().toLowerCase();if(email&&!learnerRoster.has(email))learnerRoster.set(email,email)})}}catch(rosterError){console.warn('Pilot roster unavailable',rosterError)}document.getElementById('managerLearnerOptions').innerHTML=[...learnerRoster.entries()].map(([email,name])=>'<option value=\"'+esc(email)+'\">'+esc(name)+'</option>').join('');"
  );
