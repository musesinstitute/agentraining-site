...
test('frontend open recovers and renders latest partial bank without regenerating or overwriting it',async()=>{
  ...
  assert.equal(rendered.at(-1).id,partial.id);assert.ok(elements.get('qb-resume-btn'),'partial recovery must expose an explicit resume action');assert.equal(context.currentQB.id,partial.id);assert.deepEqual(requests.filter(x=>x.endpoint.includes('question-bank')).map(x=>x.body.action).sort(),['capacity','latest']);assert.equal(requests.some(x=>['start','generate_batch'].includes(x.body.action)),false);assert.equal(JSON.parse(session.get('kte_active_bank')).bankId,partial.id);
});
